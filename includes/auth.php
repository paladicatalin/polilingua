...
const DEFAULT_ADMIN_EMAIL = 'admin@polilingua.md';
const DEFAULT_ADMIN_PASSWORD_HASH = '$2y$12$./al037Z68REpLbSFV2VU.bMfRwNdUb09sqUSHZKBorK1xwlRJKJy'; // admin123
const LEGACY_DEFAULT_ADMIN_HASH = '$2y$12$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi';
...
function ensureDefaultAdminAccount(PDO $db): void {
    try {
        ...
        if (($admin['password_hash'] ?? '') === LEGACY_DEFAULT_ADMIN_HASH) {
            $update = $db->prepare("UPDATE admins SET password_hash = ? WHERE id = ?");
            $update->execute([DEFAULT_ADMIN_PASSWORD_HASH, (int)$admin['id']]);
        }
    } catch (Throwable $e) {
